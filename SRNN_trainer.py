...
class SRNNTrainer:
    ...
    def easytrain(self, brickSize, batchSize, epochs, x_train, x_val, y_train, y_val,
              printStep=10, valStep=1):
        '''
        Same as "train" just keeps vectors in gpu memory as torch tensors
        ...
        trainNumBatches = int(math.ceil((x_train.shape[1]) / batchSize))
        ...
        x_train_batches = torch.split(x_train, trainNumBatches,1)
        y_train_batches = torch.split(y_train, trainNumBatches)
    ...
    def train(self, brickSize, batchSize, epochs, x_train, x_val, y_train, y_val,
        ...
        x_train_batches = np.array_split(x_train, trainNumBatches,1)
        y_train_batches = np.array_split(y_train, trainNumBatches)
        x_val_batches = np.array_split(x_val, valNumBatches, 1)
