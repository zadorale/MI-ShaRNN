...
class ShaMIRNNTrainer:
    ...
    def train(self, brickSize, batchSize, epochs, x_train, x_val, y_train, y_val ,
        ...
        x_train , y_train , bag_size= self.torch_slicer_MIRNN(x_train,y_train,lenS,skipS)

        x_val , y_val ,_ = self.torch_slicer_MIRNN(x_val,y_val,lenS,skipS)
        ...
        for cround in range(numRounds):
            ...
            for citer in range(numIter):
                ...
                #train model
                self.trainer.easytrain(brickSize, batchSize, epochs, x_train,
                                   x_val, curr_y, y_val,
                                   printStep=1000, valStep=3)
                ...
                for i in range(0,x_val.shape[1]-1,50):
                    with torch.no_grad():
                        out = self.srnnObj.forward(x_val[:,i:i+50].to(self.device).float(),
                                                   brickSize)
            ...
            for i in range(0,x_train.shape[1],50):
                with torch.no_grad():
                    out = self.srnnObj.forward(x_train[:,i:i+50].to(self.device).float(),
                                               brickSize)
        ...
        for i in range(0,x_val.shape[1],50):
            with torch.no_grad():
                out = self.srnnObj.forward(x_val[:,i:i+50].to(self.device).float(),brickSize)
                out = smax(out).cpu().numpy()
                out = np.argmax(out,1)
                lis_out.append(out)
        out = np.concatenate(lis_out)
